...
The goal of this project is to analyze credit risk and correlation between credit default and customer specfic information such as age and marital status. A bayesian logistic regression model will be estimated and be used to answer two questions.
  ...
  2) Probability of default, male vs female
...
The dataset contains information on default payments, demographic factors, credit data, history of payment, and bill statements of credit card clients in Taiwan from April 2005 to September 2005. Data was downloaded from Kaggle ^[https://www.kaggle.com/uciml/default-of-credit-card-clients-dataset]. This project is supposed to take about 10 hours, therefore some modifications and simplifications was done to the original data before any analysis was done. These actions might not be appropriate if the goal is to estimate the best predictive model.
  ...
  5) Variable*default.payment.next.month renamed to DEFAULTsurrounds
...
## Model
The dependent variable is binary and therefore we will fit a logistic regression model using log likelihood. The configuration setup include normal priors on the coefficients (mean 0 and variance 100), `r as.integer(n.burn.iter)` burn-in iterations and `r as.integer(n.iter)` total iterations for the `r as.integer(n.chains)` chains.

From the modeling diagnostics (see Appendix \@ref(fig:appendix-b)-\@ref(tab:appendix-d)) we can observe autocorrelation and that all parameters have not converged. This indicates that we might have to run the model more iterations and change the specification.
...
```{r echo=FALSE}
# new_dat = dat[!colnames(dat) %in% c("ID", "DEFAULT")]
# mod_pred = predict(mod_glm, newdata=new_dat, type = c("response"))
# 
# myroc <- roc(dat$DEFAULT, mod_pred)
# knitr::kable(auc(myroc), format="markdown")
```
<!-- Table: ROC AUC -->
...
## Results
The performance of the model in terms of ROC AUC is acceptable. It should be possible to improve the performance by doing o more serious data analysis, better variable transformations and removing inappropriate variables nad/or variables that dows not make sense.
...
## Conclusions
It is possible to estimate a relatively good predictive model that can be used for inference. At the same time it's important to understand that the model in this analysis need to be updated based on a more serious data analysis. Conclusions should be drawn with some caution. The analysis has been done with a limited time frame.
